...
import {
  postForgotPasswordStpOne,
  postForgotPasswordStpTwo,
...
const ForgotPasswordPage = () => {
  ...
  const fetchForgotPasswordStpOne = async (values) => {
    try {
      const response = await postForgotPasswordStpOne(values);
  ...
  const fetchForgotPasswordStpTwo = async (values) => {
    try {
      const response = await postForgotPasswordStpTwo(values);
      console.log(response.data);
      if (response.data.success) {
    ...
    <div className=" border  w-full h-full flex flex-col-reverse sm:flex-row gap-5 sm:gap-0 ">
      ...
      <div className=" border w-full sm:w-[58.5%] flex items-center pt-23 lg:pr-[calc(8.75%+1.5px)] px-4 ">
        ...
        >
          <ForgotPasswordForm
            step={step}
            submitFuncOne={fetchForgotPasswordStpOne}
            submitFuncTwo={fetchForgotPasswordStpTwo}
          />
        </motion.div>
      </div>
    </div>
