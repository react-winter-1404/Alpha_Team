      ...
      <div className={`p-2 flex flex-col justify-between flex-1 ${isRow ? 'h-full py-2' : 'gap-4'}`}>
        <div>
          <Link to={`/courses/${id}`} className="block">
            <h3 className="text-[20px] md:text-[24px] h-[40px] mb-2.5 font-bold hover:text-accent transition-colors line-clamp-1">{title}</h3>
          </Link>
          <p className="text-[14px] md:text-[16px] w-[90%] h-[50px] text-muted line-clamp-2 mb-4">{discribtion}</p>
        </div>
        ...
        <div className="flex justify-between items-center mt-auto pt-2">
          ...
          <div className="w-[120px] flex justify-between items-center">
            ...
            <button onClick={handleDislike} disabled={isDisliking} className={`w-[50px] flex justify-between items-center cursor-pointer hover:scale-105 active:scale-95 transition-transform ${isDisliking ? 'opacity-60' : ''} ${userDisliked ? 'text-danger font-bold scale-105' : ''}`}>
